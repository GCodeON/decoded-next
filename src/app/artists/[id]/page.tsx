...
export default function Artist({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    const getAlbums = async() => {
      try {
        const albums = await spotify.getArtistAlbums(id);
        if(albums) {
    ...
    <div className="flex justify-center py-8">
      {albums ? (
       <div className='grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 sm:gap-4 p-2 sm:p-6'>
        ...
        <LoadingSpinner message="Loading albums..." fullHeight />
      )}
    </div>
  )
}